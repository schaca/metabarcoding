...
~/Programs/FastQC/fastqc ./*.fastq -o $OUT_DIR/0_fastqc/
...
for file in ./*R1_001.fastq ; do
    ...
    illuminapairedend --score-min=40 -r "$filename"_R2_001.fastq "$filename"_R1_001.fastq > $OUT_DIR/1_merge/$samplename.merged.fastq
    ...
    ~/Programs/ncbi-blast-2.5.0+/bin/blastn -query $OUT_DIR/6_clean/$samplename.aligned.trimmed.uniq.c10.l100.L300.clean.fasta -db ~/Documents/Databases/12S_Turkey/database/localdb.fasta -max_target_seqs 1 -outfmt "6 qseqid sseqid length pident" -out $OUT_DIR/7_lblastn/$samplename.c10.identifications
    ~/Programs/ncbi-blast-2.5.0+/bin/blastn -query $OUT_DIR/6_clean/$samplename.aligned.trimmed.uniq.c5-10.l100.L300.clean.fasta -db ~/Documents/Databases/12S_Turkey/database/localdb.fasta -max_target_seqs 1 -outfmt "6 qseqid sseqid length pident" -out $OUT_DIR/7_lblastn/$samplename.c5-10.identifications
    ~/Programs/ncbi-blast-2.5.0+/bin/blastn -query $OUT_DIR/6_clean/$samplename.aligned.trimmed.uniq.c3-5.l100.L300.clean.fasta -db ~/Documents/Databases/12S_Turkey/database/localdb.fasta -max_target_seqs 1 -outfmt "6 qseqid sseqid length pident" -out $OUT_DIR/7_lblastn/$samplename.c3-5.identifications
    echo -e "Identifications are recorded > $OUT_DIR/7_blastn"

    echo "Parsing BLAST results"
    mkdir $OUT_DIR/8_results/$samplename
    for file in $OUT_DIR/7_lblastn/$samplename.*; do
